...
namespace Knowledge.Models
{
    ...
    public static partial class Extensions
    {
        ...
        public static SearchSchema AddFields(this SearchSchema schema, IList<Azure.Search.Documents.Indexes.Models.SearchField> fields)
        {
            ...
            {
                if ( field.Type == SearchFieldDataType.Complex)
                {
                    foreach (var subfield in field.Fields)
                    {
                        string subfieldName = field.Name + '/' + subfield.Name;

                        schema.Fields[subfieldName] = subfield.ToSearchField();
                        schema.Fields[subfieldName].Name = subfieldName;
                    }
                    continue;
                }
                else
